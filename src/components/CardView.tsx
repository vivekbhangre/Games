...
export default function CardView({ card, playable = false, isIllegal = false, onClick, className, overlap, hidden }: CardViewProps) {
  ...
  return (
    <motion.div
      ...
      onClick={() => onClick?.()}
      className={cn(
        "relative w-16 h-24 sm:w-20 sm:h-28 md:w-24 md:h-36 rounded-xl border border-stone-300 dark:border-stone-200 shadow-md flex flex-col justify-between p-1.5 sm:p-2 bg-white dark:bg-stone-50 select-none transition-opacity",
        isRed ? "text-red-600 dark:text-red-700" : "text-slate-900 dark:text-slate-800",
        overlap && "-ml-6 sm:-ml-8 md:-ml-12 hover:z-10",
        isIllegal && "opacity-40 brightness-75 cursor-not-allowed",
        playable && "cursor-pointer shadow-lg hover:shadow-xl z-0 hover:z-20",
    ...
    >
      ...
      <div className="flex flex-col items-center leading-none w-4 sm:w-5">
        <span className="text-sm sm:text-lg md:text-xl font-bold tracking-tighter">{card.rank}</span>
        <span className="text-xs sm:text-sm md:text-base">{card.suit}</span>
      </div>

      {/* Center Large Suit (optional for style) */}
      <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
        <span className="text-4xl sm:text-5xl md:text-6xl">{card.suit}</span>
      </div>
